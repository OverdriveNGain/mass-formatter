...
# 			if i == j:
# 				continue
# 			if blobs[i] == None or blobs[j] == None:
# 				continue
# 			if blobs[i] & blobs[j] != set():
# 				blobs[i] = blobs[i] | blobs[j]
# 				blobs[j] = None
# 	return [i for i in blobs if i != None]
...
def start():
	...
	deleteorig = input("    Delete originals? (y/n):").lower() == "y"
	iterator = 0
